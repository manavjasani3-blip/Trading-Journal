...
try:
    ...
    st.error(f"⚠️ ગૂગલ શીટ કનેક્શન એરર: શું તમે secrets.json ફાઈલ સાચી જગ્યાએ મૂકી છે? Error: {e}")
    st.stop()
    ...
    with tab1:
        ...
        with dash_page1:
            ...
            if not df.empty:
                ...
                biggest_loss = df['Actual_PnL'].min()
    ...
    with tab2:
        ...
        with st.form("new_trade_form"):
            ...
            with col_a:
                ...
                strategy = st.text_input("Strategy Used (e.g. EMA Breakout)")
            ...
            with r_col1:
                ...
                r2 = st.checkbox("2. Held trade until Target or SL (Patience)")
            with r_col2:
            ...
            if submitted:
                ...
                # ગૂગલ શીટમાં નવી લાઇન (Row) એડ કરવી
                new_row = [
                    str(trade_date), st.session_state.username, symbol, trade_type, 
                ...
                st.success(f"✅ Trade successfully saved to Google Sheets for {st.session_state.username}!")
                st.rerun() # પેજ રીફ્રેશ કરવા માટે
    ...
    with tab3:
        ...
        if not df.empty:
            if st.button("Generate My AI Analysis"):
                with st.spinner("AI is analyzing your journal directly from Google Sheets..."):
                    st.markdown(f"""
                    <div style='background-color: #1a0b2e; padding: 20px; border-radius: 10px; border-left: 5px solid #FF0055;'>
                        ...
                        <p><b>Data Synced:</b> Your total {len(df)} trades have been analyzed.</p>
                        <p><b>🔍 AI Observation:</b> Focus on the 'Rules Followed' column. If 'Patience' is missing frequently, you are exiting trades too early due to fear.</p>
                    </div>
                    """, unsafe_allow_html=True)
        else:
